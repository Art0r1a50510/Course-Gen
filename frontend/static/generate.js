...
function setupEventListeners() {
    ...
    // PDF курс: иконка + выбор файла + отправка
    const pdfIcon = document.getElementById('pdf-upload-icon');
    if (pdfIcon) {
    ...
    if (pdfInput) {
        pdfInput.addEventListener('change', function (e) {
            ...
            if (file) window.handleCourseCreationFromPDF(file);
        });
    }

    // Кнопка удаления курса на странице course-detail 
    if (window.location.pathname.includes('/course-detail')) {
        setTimeout(function () {
            ...
            if (delBtn && currentCourseId) {
                delBtn.onclick = function () { window.deleteCourse(currentCourseId); };
            }
        }, 700);
    }
}
...
function createCourseCard(course) {
    ...
    <button onclick="openCourse(${course.id})"
        class="flex-1 bg-primary text-cobblestone-blue py-2 px-4 rounded-lg text-sm font-bold hover:scale-105 transition-transform flex items-center justify-center gap-2"
        aria-label="Открыть курс"
    >
        <span class="material-symbols-outlined text-sm">visibility</span>
        Открыть
    </button>
    <button onclick="downloadCoursePdf(${course.id})"
        class="flex items-center gap-1 bg-gray-100 text-cobblestone-blue py-2 px-3 rounded-lg text-sm font-bold hover:scale-105 transition-transform"
        aria-label="Скачать PDF"
    >
        <span class="material-symbols-outlined text-sm">download</span>
    </button>
    <button onclick="deleteCourse(${course.id})"
        class="flex items-center gap-1 bg-red-600 text-white px-3 py-2 rounded-lg text-sm font-bold hover:scale-105 transition-transform"
...
function showResult(message, type) {
    ...
    resultBox.className = `fixed bottom-6 right-6 max-w-lg rounded-lg border p-4 shadow-lg text-sm z-50 ${type === 'success'
...
window.handleCourseCreationFromPDF = function (file) {
    ...
    fetch('/api/generate-course-from-pdf', {
        ...
        .then(({ ok, data }) => {
            if (ok && data.success) {
                showResult(`✅ Курс "${data.title}" успешно создан! Перенаправление...`, 'success');
                setTimeout(() => { window.location.href = '/my-courses'; }, 2000);
            } else {
                showResult('❌ Ошибка при создании курса из PDF: ' + (data.detail || 'Неизвестная ошибка'), 'error');
            }
        })
        .catch(error => {
            showResult('❌ Ошибка сети при создании курса', 'error');
        });
};
window.deleteCourse = function (courseId) {
    const token = localStorage.getItem('access_token');
    if (!token) {
        alert('Для удаления курса нужно войти в систему');
        return;
    }
